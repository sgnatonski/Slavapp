...
namespace SlavApp.Resembler
{
    public class VPTree
    {
        ...
        private static List<Node> RecurseVPTree(ulong[] S, List<Node> list, Func<ulong, ulong, int> distance, int nb)
        {
            ...
            {
                var bucket = new Node[listLength];
                for(i = 0; i < listLength; i++)
                {
                    bucket[i] = new Node() { i = list[i].i };
                }
        ...
        public IEnumerable<Resembler.PriorityQueue.PItem> SearchVPTree(ulong q, int n = 1, int τ = int.MaxValue)
        {
        ...
        private static Func<Node, Node, int> distanceComparator = (a, b) => b.dist - a.dist;
